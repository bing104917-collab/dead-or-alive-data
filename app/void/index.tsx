...
const CONTRIBUTORS = ['GiserWong', 'SunGiavin', 'OpenSource', 'Expo', 'React Native'];
...
function RainColumn({ index }: { index: number }) {
  ...
  return (
    ...
    >
      <Text style={[styles.rainText, { fontSize: columnData.fontSize }]}>{columnData.chars}</Text>
    </Animated.View>
  );
}
...
function ContributorName({ name, index }: { name: string; index: number }) {
  ...
  const config = useMemo(() => {
    ...
    const top = 100 + Math.random() * (height - 200);
  ...
  return (
    <Animated.View style={[styles.contributorContainer, { top: config.top }, animatedStyle]}>
      <Text style={[styles.contributorText, { fontSize: config.fontSize }]}>{name}</Text>
    </Animated.View>
  );
}
      ...
      <View style={styles.overlay} pointerEvents="box-none">
        ...
        <View style={styles.hudContainer} pointerEvents="box-none">
          <View style={[styles.hudSection, wireframeMode && styles.wireframeHud]} pointerEvents="auto">
            ...
            <View style={styles.controlRow}>
              <View style={styles.labelRow}>
                <Text style={styles.label}>相似度检测</Text>
                <Switch
          ...
          <View
            style={[styles.hudSection, styles.mt20, wireframeMode && styles.wireframeHud]}
            pointerEvents="none"
          >
            <Text style={styles.sectionTitle}>[ 核心監測 / KERNEL ]</Text>
            <Text style={styles.statusText}>ARCH: REACT_NATIVE_REANIMATED</Text>
            <Text style={styles.statusText}>
              RENDER_MODE: {wireframeMode ? 'WIREFRAME' : 'CYBER_GLITCH'}
            </Text>
            <Text style={styles.statusText}>
              GRAVITY_INTENSITY: {(gravityScale * 10).toFixed(0)}%
            </Text>
            <Text style={styles.statusText}>
